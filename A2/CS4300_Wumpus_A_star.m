...
while 1==1
    ...
    for action = 1:3
        ...
        if next_state(1)>0 ...
            ...
            nodes(num_nodes).h = feval(h_name,nodes(num_nodes).state,goal_state);
    ...
    if length(frontier) > 2
        ...
        for i = 1: length(frontier)
            if nodes(frontier(i)).cost< nodes(frontier(smaller)).cost
                smaller = i;
            end
        end
        if smaller >1 
            if smaller == length(frontier)
                frontier = [frontier(smaller),frontier(1:smaller-1)];
            else
                frontier = [frontier(smaller),frontier(1:smaller-1),frontier(smaller+1:end)];
            end
        end         
    end
end