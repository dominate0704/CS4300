...
%P 1-16 B 17-32 G 33-48 S 49-64 W 65-80
...
for i  = 1: 16
    ...
    y = floor((i-1)/4)+1;
    vars(i).var = "P"+ x+y;
    vars(i+16*1).var = "B"+ x+y;
    vars(i+16*2).var = "G"+ x+y;
...
for i = 1: 16
    ...
    if i < 16
        ...
        for j = (i+1):16
            ...
            stringG = stringG + " "+ vars(j+16*2).var;
            KB(end+1).clauses = "-"+baseG + " -" +vars(j+16*2).var;
            vectorG(end+1) = j+16*2;
            KBi(end+1).clauses = [-baseGi -(j+16*2)];
        end
    ...
    KB(end+1).clauses = "-"+ vars(i).var+" -"+ vars(i+16*2).var;
    KBi(end+1).clauses = [-i -(i+16*2)];
